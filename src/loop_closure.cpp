...
void odom_callback(const nav_msgs::Path& msg)
{
  current_odom=msg;
        ...
        outfile<<ros::Time::now()<<" "<<current_odom.poses[0].pose.position.x<<" "<<current_odom.poses[0].pose.position.y<<" "<<current_odom.poses[0].pose.position.z<<" "<<current_odom.poses[0].pose.orientation.x<<" "<<current_odom.poses[0].pose.orientation.y<<" "<<current_odom.poses[0].pose.orientation.z<<" "<<current_odom.poses[0].pose.orientation.w<<endl;
        outfile.close();
} 
...
int main(int argc, char** argv) {
  ...
  ros::NodeHandle nh;
      ...
      ros::Subscriber odom=nh.subscribe("pose_graph/path_1",1,odom_callback);
      ros::spin();
}
